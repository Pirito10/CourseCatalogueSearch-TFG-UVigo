...
Para cubrir estas limitacións, o ecosistema de Drupal conta co módulo \textit{Search API} \cite{search_api}, que proporciona unha capa de abstracción para a indexación e consulta de contido con soporte para múltiples backends:

\begin{itemize}
    \item \textbf{Backend de base de datos}: almacena o índice en táboas da base de datos relacional existente. Non require infraestrutura adicional e é adecuado para catálogos de tamaño moderado.

    \item \textbf{Apache Solr}: motor de busca de código aberto baseado en Lucene. Ofrece \textit{stemming} e análise lingüística nativos e multilingües, ponderación avanzada de resultados por relevancia e mellor rendemento con grandes volumes de datos. Require un servidor Solr externo. O backend de base de datos tamén pode incorporar \textit{stemming} a través de procesadores adicionais, pero cunha integración menos profunda e menor soporte multilingüe.

    \item \textbf{Elasticsearch}: solución distribuída con capacidades similares a Solr, orientada a contornas de alta dispoñibilidade e escalabilidade horizontal. Tamén require infraestrutura adicional dedicada.
\end{itemize}

Para este traballo optouse polo backend de base de datos de \textit{Search API}, dado que non require infraestrutura adicional, é suficiente para o volume de datos do catálogo DACEM, e permite integrar a lóxica de busca difusa directamente en PHP, empregando o índice para recuperar os títulos sobre os que aplicar a métrica de Levenshtein.
